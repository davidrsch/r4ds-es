...
3.  Crea un diagrama de dispersión de `bill_depth_mm` vs. `bill_length_mm`.
    Es decir, crea un diagrama de dispersión con `bill_depth_mm` en el eje y, y `bill_length_mm` en el eje x.
    Describe la relación entre estas dos variables.

4.  ¿Qué sucede si se crea un diagrama de dispersión de `species` vs. `bill_depth_mm`?
    ¿Cuál sería una mejor opción de geoma?
...
#|   relativas. El eje y está etiquetado "count" por defecto.
ggplot(penguins, aes(x = island, fill = species)) +
...
Desafortunadamente, ggplot2 etiqueta el eje y como "count" de forma predeterminada, pero esto se puede modificar añadiendo la capa `labs()`, donde podemos establecer la etiqueta del eje y como `"proporción"`.

```{r}
#| fig-alt: |
#|  Gráficos de barras de especies de pingüinos por isla (Biscoe, Dream y Torgersen)
#| las barras están escaladas a la misma altura, lo que lo convierte en un gráfico de frecuencias
#| relativas. El eje y está etiquetado "proporción".
ggplot(penguins, aes(x = island, fill = species)) +
  geom_bar(position = "fill") +
  labs(y = "proporción")
```
